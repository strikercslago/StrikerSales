...
export function LeadFormDialog({ open, lead, saving, onClose, onSave }: { open: boolean; lead?: Lead; saving: boolean; onClose: () => void; onSave: (draft: LeadDraft) => Promise<unknown> }) {
  ...
  return <Modal open={open} label="Formulário de lead" onClose={onClose} busy={saving}><form className="modal lead-form-modal" onSubmit={save}><header><div><p className="eyebrow">{lead ? "ATUALIZAR REGISTRO" : "CADASTRO MANUAL"}</p><h2>{lead ? "Editar lead" : "Novo lead"}</h2></div><button type="button" className="icon-button" aria-label="Fechar" disabled={saving} onClick={onClose}><CloseIcon /></button></header>
    ...
    <footer><button type="button" className="secondary-button" disabled={saving} onClick={onClose}>Cancelar</button><button className="primary-button" disabled={saving}>{saving ? "Salvando..." : lead ? "Salvar alterações" : "Criar lead"}</button></footer></form></Modal>;
}
