...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const syncPlayback = () => {
      if (preference.matches) {
        ...
        wallpaperRef.current?.removeAttribute("src");
        wallpaperRef.current?.load();
        setWallpaperPaused(true);
      } else {
        if (wallpaperRef.current) wallpaperRef.current.src = "/media/login-wallpaper.mp4";
  ...
  const toggleWallpaper = () => {
    if (wallpaperPaused) {
      if (wallpaperRef.current && !wallpaperRef.current.getAttribute("src")) wallpaperRef.current.src = "/media/login-wallpaper.mp4";
      void wallpaperRef.current?.play().then(() => setWallpaperPaused(false)).catch(() => setWallpaperPaused(true));
    } else {
      wallpaperRef.current?.pause();
      setWallpaperPaused(true);
